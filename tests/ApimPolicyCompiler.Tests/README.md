# APIM Policy Compiler Test Harness
...
```bash
dotnet build tests/ApimPolicyCompiler.Tests/ApimPolicyCompiler.Tests.csproj --no-restore -warnaserror /p:AnalysisMode=AllEnabledByDefault /p:EnforceCodeStyleInBuild=true
```
